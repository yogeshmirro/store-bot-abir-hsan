...
class Config(object):
  ...
  VERIFY_KEY = os.environ.get("VERIFY_KEY", "MjM5MQ= MjM5Mm= MjM5km= MjM5kM= MjMMkm= MjMMkM= m5M5Mm= ljM5Mm= MaM5Mm= abM5Mm= MiM8lm= MJ3Mmr Mj00Mm=").split()#set plz don't use special character like @#$%* etc us only letter number and = ,To use this EARNING var value must be true.multiple key separted by space,if u want to use pre shorted link then verify_key and verify_link must be at same index. example if 'unfhd" this key is related to https://www.shorted.link then and verify_key is at index 1 in string(separated by space) then verify_link must be at index 1 in verify_link string
  VERIFY_LINK = os.environ.get("VERIFY_LINK","https://t.me/Files_storesbot?start=storebot_MjM5km==").split()#To use this EARNING var value must be True. multiple verification link separted by space. these links can be shorted links which is all related to VERIFY_KEY 
